...
void Game::Init(int framebufferWidth, int framebufferHeight)
{
    ...
    Shader effectsShader = ResourceManager::GetShader("postprocessing");
    Effects = new PostProcessor(effectsShader, this->Width, this->Height);
    ...
    // audio
    SoundEngine->play2D(FileSystem::getPath("resources/audio/breakout.ogg").c_str(), true);
}
        ...
        if (!box.Destroyed)
        {
            ...
            if (std::get<0>(collision)) // if collision is true
            {
                ...
                if (!box.IsSolid)
                {
                    ...
                    SoundEngine->play2D(FileSystem::getPath("resources/audio/bleep.ogg").c_str(), false);
                }
                else
                {   // if block is solid, enable shake effect
                    ShakeTime = 0.05f;
                    Effects->Shake = true;
                    SoundEngine->play2D(FileSystem::getPath("resources/audio/solid.wav").c_str(), false);
                }
